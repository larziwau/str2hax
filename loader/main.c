...
static int valid_memory_payload(u32 *payload)
{
    ...
    if (!(((payload_end > LOW_RAM_BASE) && (payload_end < LOW_RAM_END)) ||
          ((payload_end > HIGH_RAM_BASE) && (payload_end < HIGH_RAM_END)))) {
        printf("  SIZE FAIL.\n");
        return 0;
    }
...
static int decompress_memory_payload(u32 *payload)
{
    ...
    size_t result = tinfl_decompress_mem_to_mem(code_buffer, 0x92000000 - (unsigned int)code_buffer,
                                                payload, size, TINFL_FLAG_PARSE_ZLIB_HEADER);
